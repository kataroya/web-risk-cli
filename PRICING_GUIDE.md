...
```mermaid
xychart-beta
    title "월 1,000만 건 — 악성 비율별 연 비용 (USD)"
    x-axis ["0.003%", "0.1%", "1%", "5%", "10%", "20%"]
    y-axis "연 비용 (USD)" 0 --> 1300000
    bar [330, 6150, 60150, 300150, 600150, 1200150]
```

> 악성 비율 5% 이상이면 비용이 급격히 증가합니다. 아래 로그 스케일 차트로 저비율 구간도 확인하세요:

```mermaid
xychart-beta
    title "악성 비율별 연 비용 — 저비율 구간 확대 (USD, ~$70K)"
    x-axis ["0.003%", "0.1%", "1%"]
    y-axis "연 비용 (USD)" 0 --> 70000
    bar [330, 6150, 60150]
```
...
악성 비율 10% (Google 추정) 기준, 자체 DB 커버율에 따른 비용 변화:
...
```mermaid
xychart-beta
    title "자체 DB 커버율별 연 비용 (악성 10% 기준, USD)"
    x-axis ["0%", "50%", "70%", "90%", "95%"]
    y-axis "연 비용 (USD)" 0 --> 650000
    bar [600000, 300000, 180000, 60000, 30000]
```
