...
flowchart TD
    ...
    B1 -->|"No"| SAFE_LOCAL["SAFE\n(API 호출 없음)"]
    B1 -->|"Yes"| API["hashes.search\n($50/1K)"]
    API --> API_R{"위협 확정?"}
    API_R -->|"Yes"| BLOCK
    API_R -->|"No"| SAFE_API["SAFE\n(false positive)"]
...
```mermaid
flowchart LR
    URL["URL 10M/월"] --> OWN["자체 DB\n선필터링"]
    OWN -->|"자체 검출\n(70~90%)"| BLOCK_OWN["🚫 자체 차단\n(무료)"]
    OWN -->|"미검출\n(10~30%)"| WR["Web Risk\nUpdate API"]
    WR -->|"로컬 SAFE"| SAFE["✅ 허용"]
    WR -->|"매칭 → 확인"| HASH["hashes.search\n(유료)"]
    HASH --> RESULT["확정"]
...
flowchart TD
    subgraph "이해관계 교차점"
        G_WANT["Google: $250K ARR 확보\n→ Submission 구독 원함"]
        B_WANT["베스핀: 마진 + 서비스 수주\n→ 쿼타 분배 + 구축 원함"]
        C_WANT["고객: 최소 비용 최대 효과\n연예산 5천만~1억"]
    end

    G_WANT --> TENSION1["⚡ 갈등: 최소 주문 $250K\n고객 1사 예산(5천만~1억)과\n비슷하거나 초과"]
    B_WANT --> TENSION2["⚡ 갈등: 선투자 리스크\n$250K 선구매 후 고객 이탈 시?"]
    C_WANT --> TENSION3["⚡ 갈등: Submission 구독 필수\n직접 PAYG 불가 → 중개 필요"]

    TENSION1 --> SOLUTION["💡 해결:\n베스핀이 $250K 일괄 구매\n→ 여러 고객에 쿼타 분배\n→ 고객당 비용 ↓\n→ Google ARR ↑"]
    TENSION2 --> SOLUTION
    TENSION3 --> SOLUTION
...
```mermaid
flowchart TD
    START["현재 상황\n고객: 월 1천만 건, 예산 5천만~1억\nGoogle: $250K 패키지 제안\n베스핀: Premier Partner"]

    START --> S1["Step 1: 고객 미팅\n• 자체 DB 커버율 확인\n• 악성 URL 대응 방식 파악\n• Submission 니즈 검증"]
    S1 --> S2["Step 2: 비용 시뮬레이션\n악성비율 × 자체커버율 → 실비용 산출"]
    S2 --> S3{"예산 내?"}

    S3 -->|"Yes"| S4A["Step 3A: 단독 계약\n고객이 $250K 직접 or 베스핀 경유"]
    S3 -->|"No"| S4B["Step 3B: 다중 고객 분배\n베스핀이 $250K 일괄 → 쿼타 분배"]

    S4A --> S5["Step 4: PoC 진행\n본 클라이언트로 검증"]
    S4B --> S5

    S5 --> PKG["Step 5: 패키지 구성\n─────────────────\n• Submission 쿼타 배분\n• Update API 구축·운영\n• GCP 보안 인프라\n• 기술 지원 SLA"]

    PKG --> WIN["Win-Win-Win\n─────────────\nGoogle: $250K ARR + 생태계 확대\n베스핀: 구축·운영 수익 + 여러 고객 록인\n고객: 글로벌 TI + 브라우저 차단 능력 확보"]
